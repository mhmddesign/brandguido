...
const InteractiveLogo = ({ className = "" }: { className?: string }) => {
  // Animation variants for the stroke drawing effect
  const draw = {
    ...
    visible: (i: number) => ({
      ...
      opacity: 1,
      transition: {
        pathLength: { delay: i * 0.1 + 0.5, type: "spring", duration: 2, bounce: 0 },
  ...
  return (
    ...
    >
      ...
      <motion.path
        ...
        custom={6}
        fill="currentColor"
        d="M94.35,25.64c.12.61.17,1.23.13,1.85-.03.45-.1.89-.22,1.33-.49,1.8-1.63,3.4-3.37,4.15-2.02.88-4.44.63-6.34-.43-2.06-1.15-3.34-3.27-3.29-5.65.02-.89.21-1.78.5-2.62.87-2.49,3.71-4.04,6.23-4.13,3.25-.12,5.75,2.5,6.36,5.51Z"
      />
      ...
      >
        <motion.path
          ...
          fill="currentColor"
          d={starPath}
          className="text-accent-purple" // Star color
        ...
        <motion.path
          d={starPath}
          fill="currentColor"
          className="text-accent-purple opacity-50 blur-sm pointer-events-none"
        />
      </motion.g>
    </motion.svg>
  );
